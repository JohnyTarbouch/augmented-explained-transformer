...
def _set_run_overrides(cfg: dict[str, Any], seed: int, run_id: str, stages: list[str]) -> None:
    ...
    if "train" in stages:
        ...
        for section in MODEL_PATH_SECTIONS:
            if section in cfg:
                ...
                if section_cfg.get("model_path") in (None, ""):
                    section_cfg["model_path"] = training.get("output_dir")
                    cfg[section] = section_cfg


def main() -> None:
    parser = argparse.ArgumentParser(description="Run multiple seeds and aggregate results.")
    parser.add_argument("--config", default="configs/base.yaml")
    parser.add_argument("--seeds", default="13,42,1337")
    parser.add_argument(
        "--stages",
        default="faithfulness,sanity",
        help="Comma-separated list of stages to run.",
    )
    parser.add_argument(
        "--base-run-id",
        default=None,
        help="Base run id prefix (default: config project.run_id).",
    )
    parser.add_argument(
        "--aggregate",
        action="store_true",
    ...
    base_run_id = args.base_run_id or cfg.get("project", {}).get("run_id", "run")
    ...
    for seed in seeds:
        ...
        _set_run_overrides(cfg_seed, seed, run_id, stages)
