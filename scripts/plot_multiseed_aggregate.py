...
def _plot_sanity(summary: dict, out_path: Path) -> bool:
    ...
    for key, label, color in metrics:
        ...
        plt.fill_between(x, [v - s for v, s in zip(vals, stds)], [v + s for v, s in zip(vals, stds)],
...
def _plot_faithfulness(summary: dict, out_path: Path) -> bool:
    ...
    def plot_curve(ax, curve, rand_curve, title):
        ...
        ax.fill_between(x, [m - s for m, s in zip(mean, std)], [m + s for m, s in zip(mean, std)],
                        color="#2b6cb0", alpha=0.15)
        if rand_curve is not None:
            rmean, rstd = rand_curve
            ax.plot(x, rmean, color="#c05621", linestyle="--", label="random")
            ax.fill_between(x, [m - s for m, s in zip(rmean, rstd)], [m + s for m, s in zip(rmean, rstd)],
                            color="#c05621", alpha=0.12)
        ax.set_title(title)
